...
class VideoViewerVC: UIViewController {
    ...
    lazy var pauseBtn: UIBarButtonItem = {
        ...
        let barButton = UIBarButtonItem(barButtonSystemItem: .pause, target: self, action: #selector(VideoViewerVC.didPressPauseBtn(_:)))
    ...
    internal func setupEvents() {
        ...
        if let url = videoUrl {
            ...
            let duration = VideoHelper.getMediaDuration(url: videoUrl!)
            
            let endDate = Calendar.current.date(byAdding: .second, value: Int(floor(duration)), to: startDate)!
            
            let event = KSTimelineEvent(start: startDate, end: endDate, duration: duration, videoURL: url)
...
//        self.events.append(event)
    ...
    internal func playEvent(event: KSTimelineEvent) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(VideoViewerVC.playerDidFinishPlaying(_:)),
        ...
        self.displayLink = CADisplayLink(target: self, selector: #selector(VideoViewerVC.displayLinkDidFire(_:)))
...
extension VideoViewerVC: KSTimelineDelegate {
...
extension VideoViewerVC: KSTimelineDatasource {
